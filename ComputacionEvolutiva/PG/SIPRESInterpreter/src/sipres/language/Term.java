 ...
 * @author Camiku
 */
public class Term implements SipresConstants {
    ...
    private static LinkedList listSubTermsConst = new LinkedList<Term>();
    ...
    public Term(char value, byte type) {
    ...
    public void addChild(Term term) {
        if (this.getListChildren() == null) {
            this.setListChildren(new LinkedList());
        }
    ...
    public static boolean hasList(Term term) {
        switch (term.getType()) {
            case LIST:
            case NULL:
                return true;
    ...
    public Term whereEmptyList() {
        return whereEmptyList(this);
    }

    public static Term whereEmptyList(Term term) {
        switch (term.getType()) {
            case NULL:
                return term;
            case VARIABLE:
    ...
    public LinkedList<Term> getSubTermsConstants() {
        return getSubTermsConstants(this);
    }

    public static LinkedList<Term> getSubTermsConstants(Term term) {
        listSubTermsConst.clear();
        return getSubTermsConstants(term, listSubTermsConst);
    }

    private static LinkedList<Term> getSubTermsConstants(Term term, LinkedList<Term> list) {
        switch (term.getType()) {
            case VARIABLE:
                break;
            case NULL:
            ...
            case LIST:
                for (Iterator<Term> it = term.getListChildren().iterator(); it.hasNext();) {
                    getSubTermsConstants(it.next(), list);
                }
                break;
        }
        return list;
    ...
    private static Term changeVarByVar(HashSet<String> hash, Term term) {
        switch (term.getType()) {
            case VARIABLE:
                term.setValue(SipresSetTheory.getRandomElementSet(hash));
                break;
            case NULL:
            case TRUE:
            case FALSE:
    ...
    public Term changeAllConstantsBySameVar() {
        return changeAllConstantsBySameVar(this);
    }

    public static Term changeAllConstantsBySameVar(Term term) {
        return changeAllConstantsBySameVar(getSetVars(term), term).renameVar();
    }

    private static Term changeAllConstantsBySameVar(HashSet<String> hash, Term term) {
        switch (term.getType()) {
            case VARIABLE:
                break;
            case NULL:
            ...
            case LIST:
                for (Iterator<Term> it = term.getListChildren().iterator(); it.hasNext();) {
                    changeAllConstantsBySameVar(hash, it.next());
                }
                break;
        }
        return term;
    ...
    public Term changeConstantsBySameSetVar() {
        return changeConstantsBySameSetVar(this);
    }

    public static Term changeConstantsBySameSetVar(Term term) {
        Term term1 = changeConstantsBySameSetVar(getSetVars(term), term);
        if (!term1.hasVar()) {
            LinkedList<Term> list = term1.getSubTermsConstants();
            Random random = new Random();
            Term term2 = list.get(random.nextInt(list.size()));
            term2.setValue(getFreshVariable());
            term2.setType(VARIABLE);
        }
        return term1.renameVar();
    }

    private static Term changeConstantsBySameSetVar(HashSet<String> hash, Term term) {
        switch (term.getType()) {
            case VARIABLE:
                break;
            case NULL:
            ...
            case CONSTANT:
            case UNDEFINED:
                // We must add at least one variable
                Random random = new Random();
                if (random.nextBoolean()) {
                    if (hash.size() > 0 && random.nextBoolean()) {
                        term.setValue(SipresSetTheory.getRandomElementSet(hash));
            ...
            case LIST:
                for (Iterator<Term> it = term.getListChildren().iterator(); it.hasNext();) {
                    changeConstantsBySameSetVar(hash, it.next());
                }
                break;
        }
        return term;
    ...
    public Term changeVarConsBySetVar(HashSet<String> hash) {
        return changeVarConsBySetVar(hash, this);
    }

    public static Term changeVarConsBySetVar(HashSet<String> hash, Term term) {
        Random random = new Random();
        switch (term.getType()) {
            case VARIABLE:
                term.setValue(SipresSetTheory.getRandomElementSet(hash));
            ...
            case SUCCESSOR:
                if (random.nextBoolean()) {
                    ...
                    changeVarConsBySetVar(hash, term.getChild(0));
                }
                break;
            case FUNCTOR:
            case LIST:
            ...
            case EQUAL_SYMBOL:
                for (Iterator<Term> it = term.getListChildren().iterator(); it.hasNext();) {
                    changeVarConsBySetVar(hash, it.next());
                }
                break;
            case NULL:
            case NATURAL:
    ...
    public Term changeFunVarConsBySetVar(HashSet<String> hash) {
        return changeFunVarConsBySetVar(hash, this);
    }

    public static Term changeFunVarConsBySetVar(HashSet<String> hash, Term term) {
        Random random = new Random();
        switch (term.getType()) {
            case VARIABLE:
                term.setValue(SipresSetTheory.getRandomElementSet(hash));
            ...
            case SUCCESSOR:
                if (random.nextBoolean()) {
                    ...
                    changeFunVarConsBySetVar(hash, term.getChild(0));
                }
                break;
            case FUNCTOR:
            case LIST:
                if (random.nextDouble() < 0.25) {
                    ...
                    for (Iterator<Term> it = term.getListChildren().iterator(); it.hasNext();) {
                        changeFunVarConsBySetVar(hash, it.next());
                    }
                }
                break;
            case NULL:
    ...
    public boolean areVarRhsInLhs() {
        if (this.getNumberChildren() == 2) {
            ...
            return SipresSetTheory.isSubSet(hashRhs, hashLhs);
        } else {
            return false;
        }
    }
        ...
        switch (getType()) {
            ...
            case FUNCTOR:
                ...
                if (hashCommutativeOperators.contains(getValue())) {
                    funicoF = 1;
                    for (Term term : getListChildren()) {
                        funicoF = Math.max(funicoF, term.calculateFUNICO(hashCommutativeOperators));
                    }
                } else {
                    funicoF = getArity();
                    for (Term term : getListChildren()) {
                        funicoF += term.calculateFUNICO(hashCommutativeOperators);
                    }
                }
                return funicoF;
            /*
            ...
            case LIST:
                int funicoL = 1;
                if (getListChildren() != null) {
                    for (Term term : getListChildren()) {
                        funicoL += term.calculateFUNICO(hashCommutativeOperators);
                    }
                }
                return funicoL;
            case EQUAL:
            case EQUAL_SYMBOL:
                int funico = getChild(0).calculateFUNICO(hashCommutativeOperators);
                funico += getChild(1).calculateFUNICO(hashCommutativeOperators);
                return funico;
            default:
                System.err.println("Error calculating FUNICO Term: " + getValue() + "\t" + getType());
                return 0;
        }
    }

    public int calculateConstantsFactor() {
        switch (getType()) {
            case VARIABLE:
                return 2;
            case NULL:
