 ...
 * @author Camiku
 */
public class Extractor implements SipresConstants {
    ...
    public Extractor(String positiveExamples, String background) throws LexicalException, SyntacticalException, ExampleException {
        ...
        //System.err.println(tableMainFunctors);
     ...
     */
    public static HashMap<String, Integer> getTableFunctors() {
    ...
    protected Equation generateNewEquationFull(int depth) {
        ...
        left = new Term(SipresSetTheory.getRandomElementSet(getTableMainFunctors()), FUNCTOR, root);
        root.addChild(left);
    ...
    protected Term generateNewTermFull(int depth) {
        functor = SipresSetTheory.getRandomElementSet(getTableFunctors().keySet());
        if (functor.charAt(0) == '•') {
            root = new Term(functor, LIST, null);
        } else {
            root = new Term(functor, FUNCTOR, null);
        }
        generateNewTermFull(root, depth, 0);
        return root;
    }
    ...
    protected Term generateNewTermFull(Term parent, int depth, int level) {
        ...
        for (int i = 0; i < numberChildren; i++) {
            ...
            if (level < (depth - 1)) {
                functor = SipresSetTheory.getRandomElementSet(getTableFunctors().keySet());
                if (functor.charAt(0) == '•') {
                    newTerm = new Term(functor, LIST, parent);
                } else {
                ...
                terminal = SipresSetTheory.getRandomElementSet(getTableTerminalsWithType().keySet());
                if (getTableTerminalsWithType().get(terminal) == NATURAL) {
                    newTerm = SipresSetTheory.generateTreeNumber(Integer.parseInt(terminal));
                ...
                variable = SipresSetTheory.getRandomElementSet(getTableVariables());
    ...
    protected Term generateNewTermGrow(int maxDepth, double probability) {
        ...
        root = new Term(SipresSetTheory.getRandomElementSet(getTableMainFunctors()), FUNCTOR, null);
        generateNewTermGrow(root, maxDepth, 0, probability);
        return root;
    }
    ...
    protected Equation generateRandomEquation(int depth, double probability) {
        Equation equa;
        Random random = new Random();
        if (random.nextBoolean()) {
    ...
    public LinkedList<Equation> getInitialPopulation(int popSize, int depth, double probability) {
        LinkedList<Equation> list = new LinkedList();
        for (int i = 0; i < popSize; i++) {
            list.add(generateRandomEquation(depth, probability));
        }
        return list;
    }
