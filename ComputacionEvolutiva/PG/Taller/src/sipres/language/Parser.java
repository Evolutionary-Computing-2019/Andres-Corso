 ...
 * @author Camiku
 */
public class Parser implements SipresConstants {
    ...
    public static LinkedList<Term> parsing(String source) throws SyntacticalException, LexicalException {
        ...
        try {
            ...
            // used to count the number of parameters of a list
            stackArgs.clear();
            Term node = new Term(); //"", MARK_OF_BOTTOM, null
            Term root = node;
            stack.clear();
            stack.addFirst(VAR_TERM);
            do {
                node = analyzeToken(listToken, stack, node);
                if (node == null) {
    ...
    private static Term analyzeToken(LinkedList<Token> listToken, LinkedList<Byte> stack, Term node) {
        ...
        switch (type_variable) {
            ...
            case NATURAL:
                ...
                if (type_variable == token.getType()) {
                    ...
                    return SipresSetTheory.generateTreeNumber(n, node);
                } else {
                    stack = null;
                }
                break;
